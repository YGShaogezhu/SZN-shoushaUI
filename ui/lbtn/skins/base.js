...
export function createBaseLbtnPlugin(lib, game, ui, get, ai, _status, app) {
	return {
		...
		// 支持的游戏模式
		supportedModes: ["identity", "doudizhu", "guozhan", "versus", "single", "boss", "connect"],/*●*/
		...
		initRoundUpdate() {
			...
			game.updateRoundNumber = function () {
				...
				let limit = ui.cardPile.childNodes.length || 0;
				if (ui.cardRoundTime) {
					game.broadcastAll(function (limit) {
						if(window.decadeUI) ui.cardRoundTime.updateRoundCard(limit);/*●*/
					}, limit)
				}
			};
		},
