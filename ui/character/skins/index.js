...
export function getCurrentSkinName() {
	const style = window.lib?.config?.["extension_十周年UI_newDecadeStyle"] || "off";
	return STYLE_TO_SKIN[style] || "shousha";
}
...
export function createCharacterPluginForSkin(lib, game, ui, get, ai, _status, app) {
	...
	// 默认使用十周年风格
	return createShoushaCharacterPlugin(lib, game, ui, get, ai, _status, app);
}
