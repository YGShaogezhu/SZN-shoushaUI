...
export function playerMarkSkillCharacter(id, target, name, content) {
	...
	game.broadcastAll(
		(player, target, name, content, id) => {
			if (player.marks[id]) {
				if (typeof updateExistingSkillMark === "function") updateExistingSkillMark(player, id, name, content, target);
			} else {
				if (typeof createNewSkillMark === "function") createNewSkillMark(player, id, name, content, target);
			}
			if (player.marks[id]) {
