...
const getProgressBarConfig = () => {
	...
	const configs = {
		1: {
			container: { backgroundColor: "rgba(0,0,0,0.4)", width: "530px",/*●*/ height: "12.3px", borderRadius: "1000px", boxShadow: "0px 0px 9px #2e2b27 inset,0px 0px 2.1px #FFFFD5", overflow: "hidden", border: "1.2px solid #000000", position: "fixed", left: "calc(50% - 300px)", bottom },
			progressBar: { data: 530,/*●*/ style: "background-image: linear-gradient(#fccc54 15%, #d01424 30%, #cc6953 90%);height:12.8px;" },
			clearSpecial: true,
		},
		2: {
...
export function initPrecontentUI() {
	...
	game.Jindutiaoplayer = () => {
		...
		if (lib.configOL && lib.configOL.choose_timeout){
		    let chooseTimeout = 2 * lib.configOL.choose_timeout;
		    if(lib.config.extension_十周年UI_jindutiaoST != chooseTimeout) game.saveConfig("extension_十周年UI_jindutiaoST", chooseTimeout);
		}
		...
		const interval = parseFloat(lib.config.extension_十周年UI_jindutiaoST*0.9);/*●*/
